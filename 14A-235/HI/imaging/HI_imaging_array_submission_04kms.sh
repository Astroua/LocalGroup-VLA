...
#SBATCH --job-name=M31_14A_042kms-%A-%a
#SBATCH --output=casa-M31_14A_042kms-%A-%a.out
#SBATCH --array=0-382
...
if (( $stage==1 )); then
    ...
    # Start 4 channels running on the node (4 * 8 = 32 cores)
    # This is more CPU-heavy making the PSF, etc.
    ...
    # CLEANing takes most of the time and uses fewer CPUs
    # so overload CPU usage.
    # This is well-suited for the cedar base nodes
...
export chan_path_name="HI_contsub_0_42kms"
...
for (( chan_num = $start_chan; chan_num < $end_chan; chan_num++ )); do
    ...
    (casa-release-5.4.1-32.el7/bin/mpicasa -n 8 casa-release-5.4.1-32.el7/bin/casa --rcdir ${rc_path} --nologger --nogui --logfile $scratch_path/${chan_path_name}/casa_M31_HI_14A_04kms_${chan_num}_${SLURM_JOB_ID}_$(date "+%Y%m%d-%H%M%S")_stage${stage}.log --nocrashreport -c $script_name $chan_num $param_file "${chan_path_name}"; cp -r ${chan_path_name}/channel_${chan_num}/* $scratch_path/${chan_path_name}/channel_${chan_num}/) &
    pids+=" $!"
