...
######################################################################
#
# 10/1/14 CJC: Commented out references to
# flagging uncalibrated spws, per master script
#
######################################################################
...
if (QA2_import=='Fail' or QA2_hanning=='Fail' or QA2_msinfo=='Fail' or QA2_flagall=='Fail' or QA2_calprep=='Fail' or QA2_priorcals=='Fail' or QA2_testBPdcals=='Fail' or QA2_flag_baddeformatters=='Fail' or QA2_flag_baddeformattersphase=='Fail' or QA2_checkflag=='Fail' or QA2_semiFinalBPdcals1=='Fail' or QA2_checkflag_semiFinal=='Fail' or QA2_semiFinalBPdcals2=='Fail' or QA2_solint=='Fail' or QA2_testgains=='Fail' or QA2_fluxgains=='Fail' or QA2_fluxboot=='Fail' or QA2_finalcals=='Fail' or QA2_applycals=='Fail' or QA2_targetflag=='Fail' or QA2_statwt=='Fail' or QA2_plotsummary=='Fail'):
    QA2_pipeline='Fail'
elif (QA2_import=='Partial' or QA2_hanning=='Partial' or QA2_msinfo=='Partial' or QA2_flagall=='Partial' or QA2_calprep=='Partial' or QA2_priorcals=='Partial' or QA2_testBPdcals=='Partial' or QA2_flag_baddeformatters=='Partial' or QA2_flag_baddeformattersphase=='Partial' or QA2_checkflag=='Partial' or QA2_semiFinalBPdcals1=='Partial' or QA2_checkflag_semiFinal=='Partial' or QA2_semiFinalBPdcals2=='Partial' or QA2_solint=='Partial' or QA2_testgains=='Partial' or QA2_fluxgains=='Partial' or QA2_fluxboot=='Partial' or QA2_finalcals=='Partial' or QA2_applycals=='Partial' or QA2_targetflag=='Partial' or QA2_statwt=='Partial' or QA2_plotsummary=='Partial'):
...
commentlog.write('- If your observations used a resolved calibrator source (i.e., has UV\n')
commentlog.write('  limits) but does not have standard model images in CASA, the calibrated\n')
commentlog.write('  data through the pipeline would not be accurate. In such instances,\n')
commentlog.write('  re-calibration using UV limits, or imaging the calibrator first and\n')
commentlog.write('  using the resulting image model, will be needed.\n\n')
commentlog.close()
...
wlog.write('<br>CASA version: '+casa['build']['version']+' revision #'+casa['build']['number']+' \n')
...
wlog.write('<li>Log: <a href="./logs/plotsummary.log" type="text/plain" target="_blank">link</a></li>\n')
